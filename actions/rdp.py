...
def _close_rdp(target: str, username: str, password: str):
    try:
        ...
        if not session_id:
            logging.error("Could not determine session ID on target — skipping console handoff")
        else:
            ...
            subprocess.run(ssh_handoff_payload, shell=True, capture_output=True, timeout=10)
